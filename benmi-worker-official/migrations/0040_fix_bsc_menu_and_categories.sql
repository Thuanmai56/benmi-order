...
-- 6. Ensure BSC customizations has 配料調整 (3 options) and 加價配料 (選加) (out of stock)
...
VALUES
    ...
    ('bsc_addons', 'bsc', 'addons', '✦ 加價配料 (選加)', 'checkbox', 4, '[{"name":"加香菜","price":15,"is_out_of_stock":true},{"name":"加檸檬汁","price":20,"is_out_of_stock":true}]')
ON CONFLICT(id) DO UPDATE SET
...
-- 7. Sync BSC production out-of-stock items
UPDATE menu_items SET out_of_stock_until = NULL WHERE tenant_id = 'bsc';

UPDATE menu_items 
SET out_of_stock_until = '2099-12-31 23:59:59' 
WHERE tenant_id = 'bsc' AND name IN (
    '松阪豬',
    '鳥蛋(5顆)',
    '甜不辣',
    '滷苦瓜',
