...
export async function debugKV(env: Env, url: URL): Promise<Response> {
  ...
  const d1OrdersCount = await env.DB.prepare("SELECT COUNT(*) as count FROM orders").first<{ count: number }>().catch(() => ({ count: 0 }));
  const d1TenantsCount = await env.DB.prepare("SELECT COUNT(*) as count FROM tenants").first<{ count: number }>().catch(() => ({ count: 0 }));
  ...
  return json({
    d1_orders_count: d1OrdersCount?.count || 0,
    d1_tenants_count: d1TenantsCount?.count || 0,
